...
export default function TagPostCard({post}) {
  ...
  return (
    <article>
      ...
      <div className='grid grid-cols-1 gap-2 rounded-md border-2 p-4 shadow-lg md:p-8 lg:grid-cols-3 lg:gap-6 lg:p-2'>
        <section className='hidden p-4 drop-shadow-xl transition-transform duration-200 ease-out hover:scale-105 lg:block'>
        ...
        <section className='flex flex-col gap-4 lg:my-2 lg:p-4'>
        ...
        <section className='flex flex-col justify-normal lg:my-2 lg:justify-between lg:p-4'>
          <h4 className='prose prose-base prose-slate line-clamp-2 text-gray-500 lg:prose-lg dark:prose-invert lg:line-clamp-3'>
            {excerpt}
          </h4>
          <div className='flex justify-end'>
            <Link href={`/blog/post/${post.postSlug}`}>
              <h5 className='mt-4 font-bold text-[#00ceff]'>Read Post&#8594;</h5>
            </Link>
          </div>
        </section>
      </div>
