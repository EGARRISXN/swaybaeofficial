...
import {socialMediaData} from '@/app/utils/constants'

export default function ContactSocials({mobileStyle}) {
  return (
    <div
      className={`border-base-content bg-base-200 text-base-content ${
        mobileStyle ? 'mt-12' : ''
      } rounded-3xl border-4 shadow-xl shadow-purple-800/50 ${mobileStyle ? 'py-4' : 'py-8'} px-6`}
    >
      <div className='lobster text-center text-2xl text-primary'>Let&apos;s Connect!</div>
        ...
        <ul className='mb-3 flex flex-col space-y-2'>
          {socialMediaData.map((item, index) => (
            <li key={index} className='flex items-center'>
              <Link href={item.href} className='flex items-center hover:text-secondary'>
                {item.icon}
